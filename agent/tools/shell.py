...
class ShellTaskOutputTool(Tool):
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "timeout_ms": {
                    "type": "integer",
                    "description": "block=true 时的最长等待时间（毫秒），默认 30000，最大 30000。超过 30 秒后无论任务是否完成都会返回当前进度，以便及时回复用户",
                    "minimum": 0,
                },
            },
            "required": ["task_id"],
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        # 硬限制：block 等待不能超过 30 秒。
        # 防止 LLM 传一个超大 timeout_ms（如 120000ms）导致用户长时间收不到回复。
        # 超过此限制时，工具返回当前进度和 "running" 状态，LLM 可据此回复用户后再决定是否继续等。
        MAX_BLOCK_MS = 30_000
        if block and timeout_ms > MAX_BLOCK_MS:
            logger.warning(
                "task_output block timeout_ms 从 %d 截断到 %d（防止长时间阻塞不回复）",
                timeout_ms,
                MAX_BLOCK_MS,
            )
