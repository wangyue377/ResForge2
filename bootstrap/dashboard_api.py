...
class ChatProcessor(Protocol):
    """Protocol for the agent loop's chat interface."""
...
def create_dashboard_app(
    ...
    *,
    ...
    agent_loop: ChatProcessor | None = None,
) -> FastAPI:
    ...
    chat_dir = project_root / "static" / "chat"

    if agent_loop is not None:

        @app.post("/api/chat")
        async def chat_send(payload: ChatRequest) -> ChatResponse:
            if not payload.message.strip():
                raise HTTPException(status_code=400, detail="消息不能为空")
            session_key = payload.session_key or f"web:{uuid.uuid4().hex[:12]}"
            ...
            return ChatResponse(response=response_text, session_key=session_key)

    @app.get("/chat")
    def chat_index() -> Response:
        if chat_dir.is_dir() and (chat_dir / "index.html").exists():
            html = (chat_dir / "index.html").read_text(encoding="utf-8")
            return Response(content=html, media_type="text/html")
        return Response(content="Chat frontend not found. Run build first.", status_code=404, media_type="text/plain")
