...
async def _fetch_by_channel_async(pool: McpClientPool, *, channel: str) -> list[dict]:
    ...
    for src in _iter_sources_by_channel(channel, pool._workspace):
        server = src.get("server", "")
        # 跳过未注册的 server（例如 proactive_sources.json 配了但 mcp_servers.json 没有）
        if not pool.has_server(server):
            logger.debug(
                "[mcp_sources] fetch_%s: %s 未注册，跳过", channel, server
            )
            continue
        # 2. 每个源默认调用：
        ...
        try:
            ...
            # 5. 单个源失败只记日志，不阻断其他源。
            logger.debug(
                "[mcp_sources] fetch_%s %s.%s 失败: %s",
                channel, server, get_tool, e,
            )
    return result
...
async def poll_content_feeds_async(pool: McpClientPool) -> None:
    ...
    for src in _iter_sources_by_channel("content", pool._workspace):
        ...
        if not pool.has_server(server):
            logger.debug(
                "[mcp_sources] poll_content_feeds: %s 未注册，跳过", server
            )
            continue
        try:
